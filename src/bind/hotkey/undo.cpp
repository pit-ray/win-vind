...
namespace vind
{
    namespace bind
    {
        ...
        void Redo::sprocess(unsigned int count) {
            safe_for(count, [] {
                core::InputGate::get_instance().pushup(
                        KEYCODE_LCTRL, KEYCODE_Y) ;
            }) ;
        }
        void Redo::sprocess(core::NTypeLogger& parent_lgr) {
        ...
        void Redo::sprocess(const core::CharLogger& UNUSED(parent_lgr)) {
            sprocess() ;
        }
        ...
        void Undo::sprocess(unsigned int count) {
            safe_for(count, [] {
                core::InputGate::get_instance().pushup(
                        KEYCODE_LCTRL, KEYCODE_Z) ;
            }) ;
        }
        void Undo::sprocess(core::NTypeLogger& parent_lgr) {
            if(!parent_lgr.is_long_pressing()) {
                sprocess(parent_lgr.get_head_num()) ;
                pimpl->ksr.reset() ;
            }
        ...
        void Undo::sprocess(const core::CharLogger& UNUSED(parent_lgr)) {
            sprocess() ;
        }
    }
}
