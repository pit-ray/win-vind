...
namespace vind
{
    namespace bindjsonparser {
        ...
        void load_bindings_json_parser(
                const std::string& filepath,
                const std::vector<BindedFunc::shp_t>& all_func_list,
                std::array<std::vector<std::shared_ptr<LoggerParser>>,
                    static_cast<std::size_t>(mode::Mode::NUM)>& parser_list) {
        ...
        bool setup_logger_parser_with_links(
                const BindedFunc::shp_t& func,
                const nlohmann::json& funcobj,
                const std::string& filepath) {
            //filepath is used only to print error message
