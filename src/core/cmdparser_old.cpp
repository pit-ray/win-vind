    ...
    namespace core
    {
        ...
        KeySet parse_combined_command_old(const std::string& inside_of_brackets) {
            ...
            for(auto itr = keystrset.begin() ; itr != keystrset.end() ; itr ++) {
                ...
                if(itr != keystrset.begin() && itr->length() == 1) {
                    //ascii
                    for(const auto& keycode : parse_ascii_command_old(itr->front())) {
                ...
                if(magic_ascii_.find(lowercode) != magic_ascii_.end()) {
                    for(const auto& keycode : parse_ascii_command_old(magic_ascii_[lowercode])) {
                        keyset.push_back(keycode) ;
                    }
                    continue ;
                }
        ...
        Command parse_command_old(const std::string& cmdstr) {
            ...
            for(std::size_t i = 0 ; i < cmdstr.length() ; i ++) {
                ...
                if(onechar != '<') {
                    cmd.push_back(parse_ascii_command_old(onechar)) ;
                    continue ;
                }
                ...
                cmd.push_back(parse_combined_command_old(inside_cmd)) ;
