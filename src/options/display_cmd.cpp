...
bool DisplayCmd::do_process() const
{
    ...
    auto hdc = CreateDCA(TEXT("DISPLAY"), NULL, NULL, NULL) ;
    if(!hdc) {
        write_error("CreateDC") ;
        return false ;
    }

    auto font = CreateFontIndirectA(&pimpl->lf) ;
    if(!font) {
    ...
    auto cstr = TEXT(str.c_str()) ;
    if(!TextOutA(hdc, pimpl->x, pimpl->y, cstr, lstrlen(cstr))) {
